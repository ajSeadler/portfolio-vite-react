...
const Portfolio = React.memo(() => {
  ...
  return (
    ...
    style={{ 
        width: '100%',
        objectFit: 'cover',
        transform: hoveredImage === project.id ? 'scale(1.1)' : 'scale(1)',
        transition: 'transform 0.3s ease-in-out',
    }}
        />
              {hoveredImage === project.id && (
                <div className="position-absolute top-0 start-0 w-100 h-100 d-flex justify-content-center align-items-center" style={{ background: 'rgba(255, 246, 137, 0.5)' }}>
                  <FaHandPointer className="text-white" style={{ fontSize: '5rem', color:'transparent' }} />
                </div>
              )}
            </div>
